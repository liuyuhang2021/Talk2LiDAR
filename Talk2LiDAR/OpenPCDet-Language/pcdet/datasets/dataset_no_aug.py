...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, data_dict):
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            num = data_dict['gt_names'].shape[0]
            gt_classes = np.ones((num, 1), dtype=np.float32)
            # 类别转成index
            gt_boxes = np.concatenate((data_dict['gt_boxes'], gt_classes.reshape(-1, 1).astype(np.float32)), axis=1)
            data_dict['gt_boxes'] = gt_boxes

        if data_dict.get('points', None) is not None:
            data_dict = self.point_feature_encoder.forward(data_dict)  # 明确每个点使用特征
        ...
        for key, val in data_dict.items():
            try:
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    ret[key] = np.stack(val, axis=0)  # 新建dim0
                    # 'text_token': batch_size 1 77
                    # 'text_length': batch_size 1
            except:
