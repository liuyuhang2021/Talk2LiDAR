...
class DataProcessor(object):
    def __init__(self, processor_configs, point_cloud_range, training, num_point_features):
        ...
        self.num_posint_features = num_point_features
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        if self.voxel_generator is None:
            self.voxel_generator = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,  # [0.075, 0.075, 0.2]
                coors_range_xyz=self.point_cloud_range,
                num_point_features=self.num_point_features,
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL,  # 10
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode],  # 120000，160000
            )
