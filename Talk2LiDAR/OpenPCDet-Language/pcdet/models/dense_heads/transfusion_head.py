...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        # 'heatmap': {'out_channels': 1, 'num_conv': 2}
    ...
    def predict(self, inputs):
        ...
        heatmap = dense_heatmap.detach().sigmoid()  # B 1 180 180
        padding = self.nms_kernel_size // 2  # 1
        local_max = torch.zeros_like(heatmap)  # B 1 180 180
        local_max_inner = F.max_pool2d(
            heatmap, kernel_size=self.nms_kernel_size, stride=1, padding=0
        )  # B 1 178 178
        local_max[:, :, padding:(-padding), padding:(-padding)] = local_max_inner  # 赋值给local_max
        # for Pedestrian & Traffic_cone in nuScenes
        if self.dataset_name == "nuScenes":
            local_max[ :, 8, ] = F.max_pool2d(heatmap[:, 8], kernel_size=1, stride=1, padding=0)  # 两类采用较小kernerl
        ...
        top_proposals_index = top_proposals % heatmap.shape[-1]  # 索引位置
        ...
        one_hot = F.one_hot(top_proposals_class, num_classes=self.num_classes).permute(0, 2, 1)  # B 200 10
        
        query_cat_encoding = self.class_encoding(one_hot.float())  # B 128 200
        query_feat += query_cat_encoding  # # B 128 200
        ...
        res_layer["query_feature"] = query_feat  # B 128 200

        return res_layer

    def forward(self, batch_dict):
        feats = batch_dict['spatial_features_2d']  # B 256 H W
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, pred_dicts):
        ...
        for batch_idx in range(len(gt_bboxes_3d)):  # 遍历batch
            ...
            gt_bboxes = gt_bboxes_3d[batch_idx]  # 取对应batch
            valid_idx = []
            # filter empty boxes
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        pos_assigned_gt_inds = assigned_gt_inds[pos_inds] - 1  # # 获取正样本对应的 ground truth 边界框的索引
    ...
    def decode_bbox(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        # class label
        final_preds = heatmap.max(1, keepdims=False).indices  # 类别
        final_scores = heatmap.max(1, keepdims=False).values  # 置信度
    ...
    def get_bboxes(self, preds_dicts):
        ...
        for k in range(batch_size):
            ret_dict[k]["query_feature"] = preds_dicts["query_feature"][k].permute(1, 0)  # (200, 128)
        for k in range(batch_size):
