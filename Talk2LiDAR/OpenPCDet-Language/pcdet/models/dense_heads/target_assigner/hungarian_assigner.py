...
class HungarianAssigner3D:
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, point_cloud_range):
        ...
        # bboxes: (200, 7)  gt_bboxes: (1, 7)  gt_labels: (1, 1)   cls_preds: (1, 1, 200)
        # 1. assign -1 by default
        assigned_gt_inds = bboxes.new_full((num_bboxes,), -1, dtype=torch.long)
        assigned_labels = bboxes.new_full((num_bboxes,), -1, dtype=torch.long)  # [200]
        ...
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost)  # pred中索引，gt中对应索引
        ...
        assigned_gt_inds[matched_row_inds] = matched_col_inds + 1  # pred_dict分配对应的GT box索引
        assigned_labels[matched_row_inds] = gt_labels[matched_col_inds]  # label分配为0
