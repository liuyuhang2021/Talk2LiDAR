...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        ...
        for cur_name in self.sep_head_dict:
            ...
            num_conv = self.sep_head_dict[cur_name]['num_conv']  # 每个head对应的conv层数
    ...
    def forward(self, x):
        ...
        return ret_dict  # 一个字典，包含6个branch输出的稀疏BEV的feature map
...
class VoxelNeXtHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.gaussian_ratio = self.model_cfg.get('GAUSSIAN_RATIO', 1)  # 1
        self.gaussian_type = self.model_cfg.get('GAUSSIAN_TYPE', ['nearst', 'gt_center'])  # ['nearst', 'gt_center']
        # The iou branch is only used for Waymo dataset
        self.iou_branch = self.model_cfg.get('IOU_BRANCH', False)  # false
        if self.iou_branch:
        ...
        #             }
            ...
            # {'out_channels': 1 or 2, 'num_conv': 2}
            self.heads_list.append(
                SeparateHead(
                    input_channels=self.model_cfg.get('SHARED_CONV_CHANNEL', 128),  # 128
                    sep_head_dict=cur_head_dict,
                    kernel_size=kernel_size_head,  # 1
                    init_bias=-2.19,
                    use_bias=self.model_cfg.get('USE_BIAS_BEFORE_NORM', False),  # True
                )
            )  # 每一个类别对应一个SeparateHead，每一个SeparateHead包含6路卷积层分别计算center。。。
    ...
    def assign_targets(self, gt_boxes, num_voxels, spatial_indices, spatial_shape):
        ...
        all_names = np.array(['bg', *self.class_names])  # 包含索引类别名
        for idx, cur_class_names in enumerate(self.class_names_each_head):  # 遍历每个类别head
            heatmap_list, target_boxes_list, inds_list, masks_list, gt_boxes_list = [], [], [], [], []
            for bs_idx in range(batch_size):  # batch  遍历
                cur_gt_boxes = gt_boxes[bs_idx]  # 当前帧的gt
                gt_class_names = all_names[cur_gt_boxes[:, -1].cpu().long().numpy()]  # 每一帧gt的类名

                gt_boxes_single_head = []

                for idx, name in enumerate(gt_class_names):  # 遍历类名
                    if name not in cur_class_names:  # name不在head类名跳过
                        continue
                    temp_box = cur_gt_boxes[idx]  # 取当前帧所有gt中属于该类别的box
                    temp_box[-1] = cur_class_names.index(name) + 1
                    # 当前类别的索引（在当前头类别中的位置）加1作为新的标注框的最后一个元素，表示该标注框的类别。
                    gt_boxes_single_head.append(temp_box[None, :])
                    # 将新创建的标注框添加到当前头类别内的标注框列表
    ...
    def assign_target_of_single_head(
        ...
        """
        heatmap = gt_boxes.new_zeros(num_classes, num_voxels)  # （每个head的类别数，每个batch的非空voxel）
        ...
        coord_x = (x - self.point_cloud_range[0]) / self.voxel_size[0] / feature_map_stride  # gt所属的BEV下范围
        ...
        radius = torch.clamp_min(radius.int(), min=min_radius)  # 2
        # 将计算得到的半径值转换为整数，并通过 torch.clamp_min 函数确保其不小于指定的最小值 min_radius
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            distance = self.distance(spatial_indices, center[k])  # gt center和voxel之间的距离
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, voxel_indices, spatial_shape):
        ...
        # 初始化 ret_dict，包含预测框、得分、类别和 IoU 信息
        ret_dict = [{
            'pred_boxes': [],
            'pred_scores': [],
            'pred_labels': [],
    ...
    def _get_voxel_infos(self, x):
        ...
        spatial_indices = []
        num_voxels = []
        batch_size = x.batch_size  # batch 数量
        batch_index = voxel_indices[:, 0]  # 每个体素所对应的batch

        for bs_idx in range(batch_size):
            batch_inds = batch_index==bs_idx  # bool掩码
            spatial_indices.append(voxel_indices[batch_inds][:, [2, 1]])
            # 空间坐标的顺序从 [x, y] 调整为 [y, x]，并将其添加到 spatial_indices 列表中
            num_voxels.append(batch_inds.sum())  # 当前batch的体素数量

        return spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels

    def forward(self, data_dict):
        x = data_dict['encoded_spconv_tensor']  # # 128 180 180 稀疏2D

        spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels = self._get_voxel_infos(x)
        self.forward_ret_dict['batch_index'] = batch_index  # 每个voxel属于batch的索引
