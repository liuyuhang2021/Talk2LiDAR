...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.sparse_shape = grid_size[::-1] + [1, 0, 0]  # 逆序变成 L+1，W，H

        self.conv_input = spconv.SparseSequential(
            spconv.SubMConv3d(input_channels, channels[0], 3, padding=1, bias=False, indice_key='subm1'),  # 16
            norm_fn(channels[0]),
            nn.ReLU(),
        )
    ...
    def bev_out(self, x_conv):
        ...
        indices_cat = x_conv.indices[:, [0, 2, 3]]  # batch, y, x
        spatial_shape = x_conv.spatial_shape[1:]  # 180, 180

        indices_unique, _inv = torch.unique(indices_cat, dim=0, return_inverse=True)
        features_unique = features_cat.new_zeros((indices_unique.shape[0], features_cat.shape[1]))  # m 128
        features_unique.index_add_(0, _inv, features_cat)  # 包含了去重后的坐标对应的聚合后的特征值
    ...
    def forward(self, batch_dict):
        ...
        """
        voxel_features, voxel_coords = batch_dict['voxel_features'], batch_dict['voxel_coords']  # voxel特征，voxel坐标
        ...
        batch_dict.update({
            'encoded_spconv_tensor': out,  # 128 180 180 稀疏2D
